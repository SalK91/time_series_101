...
|Cross Validation                     | $MSE: \frac{1}{n} {\sum} _{i=1} ^{n} (y_i - \hat{y}_i) ^2 \n MAE: \frac{1}{n} {\sum} _{i=1} ^{n} \|y_i - \hat{y}_i\| ^2$ | Evaluates model performance by splitting data into training and testing sets. Utilizes Mean Squared Error (MSE) or Mean Absolute Error (MAE) for evaluation. |
